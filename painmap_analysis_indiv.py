"""This script is used to analyze the painmap data for a single participant. It fits the data to an exponential and linear function and saves the data to a .pkl file.

Select datafile(s) via the file browser ui. No flexibility is implemented at the moment so these files should be .csv and of the form:
columns: timepoint, painreport, mA (floats)
with a given stim trial by row

# A three term exponential and linear fit is applied to the raw data and to normalized mA data (normalized to the max value) to allow inter-participant comparisons.
